...
void die(const char *s)
{
    if (1)
    {
        register const ssize_t k = write(STDOUT_FILENO, "\x1b[2J", 4);
        (void)k;
        register const ssize_t l = write(STDOUT_FILENO, "\x1b[H", 3);
        (void)l;
    } // handle gcc unused return value warning
...
void editorOpen(char *filename)
{
    ...
    //Try reading given filename, create if doesn't exist
    FILE *fp;
...
void editorRefreshScreen()
{
    ...
    if (1)
    {
        register const ssize_t k = write(STDOUT_FILENO, ab.b, ab.len);
        (void)k;
    }
    abFree(&ab);
}
